...
const DeliveryRegisterForm: React.FunctionComponent<IProsDelivery> = (
  props
) => {
  ...
  const [selectValueSupplier, setSelectValueSupplier] = React.useState(
    props.delivery.supplierId
  );
  const [suppliers, setSuppliers] = React.useState<ISupplier[]>(
    props.suppliers
  );

  const [selectValueWarehouse, setSelectValueWarehouse] = React.useState(
    props.delivery.warehouseId
  );
  const [warehouses, setWarehouses] = React.useState<IWarehouse[]>(
    props.warehouses
  );

  const [selectValueProduct, setSelectValueProduct] = React.useState(
    props.delivery.productId
  );
  const [products, setProduct] = React.useState<IProduct[]>(props.products);

  const [selectValueCustomer, setSelectValueCustomer] = React.useState(
    props.delivery.clientId
  );
  const [clients, setCustomer] = React.useState<ICustomer[]>(props.customers);

  const [addressOrigin, setAddressOrigin] =
    React.useState<google.maps.LatLngLiteral>(props.addressOrigin);
  const [addressDestiny, setAddressDestiny] =
    React.useState<google.maps.LatLngLiteral>(props.addressDestiny);
  ...
  const handleGetLocationCustomer = async (clientId: number) => {
    ...
    const clientAddress = await processManualLocation(
      `${client?.address}${client?.address2}${client?.city}${client?.state}${client?.country}${client?.zip}`
    );
    setAddressDestiny({
  ...
  const handleGetLocationWarehouse = async (warehouseId: number) => {
    ...
    const warehouseAddress = await processManualLocation(
      `${warehouse?.address}${warehouse?.address2}${warehouse?.city}${warehouse?.state}${warehouse?.country}${warehouse?.zip}`
    );
    setAddressOrigin({
        ...
        <Form ref={formRef} onSubmit={handleSubmit}>
          <Grid container spacing={1}>
            ...
            <Grid item xs={12} style={{ display: "grid" }}>
              <label>Cliente</label>
              <select
                value={selectValueCustomer}
                onChange={(e) =>
                  handleGetLocationCustomer(Number(e.target.value))
                }
              >
            ...
            <Grid item xs={6} style={{ display: "grid" }}>
              ...
              <select
                ...
                onChange={(e) => setSelectValueProduct(Number(e.target.value))}
                style={{ width: "100%" }}
              >
            ...
            <Grid item xs={6} style={{ display: "grid" }}>
              ...
              <select
                value={selectValueSupplier}
                onChange={(e) => setSelectValueSupplier(Number(e.target.value))}
              >
            ...
            <Grid item xs={6} style={{ display: "grid" }}>
              ...
              <select
                value={selectValueWarehouse}
                onChange={(e) =>
                  handleGetLocationWarehouse(Number(e.target.value))
                }
              >
